...
// テーマ名
const themeName = 'exroad';

// 画像をコピーする関数
function copyImages() {
    const srcImgDir = resolve(__dirname, 'src/img');
    const destImgDir = resolve(__dirname, `dist/wp-content/themes/${themeName}/assets/img`);
...
function copyPHPFiles() {
    ...
    const destDir = resolve(__dirname, `dist/wp-content/themes/${themeName}`);
...
// 削除処理を追加する関数
function deleteFromDist(file) {
    const relativePath = path.relative(resolve(__dirname, 'public'), file);
    const destPath = resolve(__dirname, `dist/wp-content/themes/${themeName}`, relativePath);
...
export default defineConfig({
    build: {
        outDir: `dist/wp-content/themes/${themeName}/assets`,
        emptyOutDir: false,
        assetsDir: '',
        copyPublicDir: false,
        rollupOptions: {
            ...
            output: {
                entryFileNames: '[name].js',    // JSファイルの名前をそのままにする
                chunkFileNames: '[name].js',
                assetFileNames: '[name].[ext]', // CSSファイルの名前をそのままにする
            },
        },
    },
    plugins: [
        {
            ...
            buildStart() {
                const cssDir = resolve(__dirname, `dist/wp-content/themes/${themeName}/assets/css/css`);
                const jsDir = resolve(__dirname, `dist/wp-content/themes/${themeName}/assets/js/js`);
